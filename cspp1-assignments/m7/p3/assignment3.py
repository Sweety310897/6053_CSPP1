...
def payingdebtoffina_year(balanc_e, annualinterest_rate):
    '''This is to calculatr balance'''
    init_balance = balanc_e
    monthlyinterest_rate = annualinterest_rate/12
    lower = init_balance/12
    upper = (init_balance * (1 +monthlyinterest_rate)**12)/12.0
    epsilon = 0.03
    while abs(balanc_e) > epsilon:
        ...
        if balanc_e > epsilon:
            lower = monthlypayment_rate
        elif balanc_e < -epsilon:
            upper = monthlypayment_rate
        else:
            break
    return str(round(monthlypayment_rate, 2))
def main():
    ...
    print('lowest payment rate:', payingdebtoffina_year(data[0], data[1]))
